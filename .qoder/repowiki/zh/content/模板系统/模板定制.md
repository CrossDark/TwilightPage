...
## 简介
本指南面向希望基于 TwilightPage 模板进行深度定制与扩展的用户。文档围绕以下目标展开：如何扩展现有模板功能（新增布局选项、修改样式系统、集成第三方组件）、模板继承与自定义模板的开发流程、主题变体与功能模块的创建范式、技术限制与约束、测试与兼容性策略、升级与维护路径，以及模板分享与发布的原则。为保证可操作性，文档在每个涉及具体实现的章节均给出"章节来源"定位到仓库中的实际文件。
...
- [template/Makefile:1-35](file://template/Makefile#L1-L35)

**章节来源**
- [template/README.md:1-38](file://template/README.md#L1-L38)
...
- 主模板与导出
  ...
  - 参考路径：[src/twilightpage.typ:17-63](file://src/twilightpage.typ#L17-L63)
...
- 构建与运行
  ...
  - 参考路径：[template/Makefile:1-35](file://template/Makefile#L1-L35)
...
participant T as "twilightpage.typ"
...
```mermaid
graph LR
TPG["twilightpage.typ"] --> MTH["math.typ"]
TPG --> RFS["refs.typ"]
TPG --> NTS["notes.typ"]
TPG --> FIG["figures.typ"]
TPG --> LYT["layout.typ"]
TPG --> CSS1["assets/tufted.css"]
TPG --> CSS2["assets/custom.css"]
```
...
- [src/twilightpage.typ:21-48](file://src/twilightpage.typ#L21-L48)
- [template/assets/tufted.css:1-166](file://template/assets/tufted.css#L1-L166)
...
- 参考路径
  - [src/twilightpage.typ:36-62](file://src/twilightpage.typ#L36-L62)
...
### G. 模板重命名更新说明
**重要更新**：模板文件已从 `tufted.typ` 重命名为 `twilightpage.typ`

- 影响范围
  - 核心模板文件：`src/tufted.typ` → `src/twilightpage.typ`
  - 包入口点：`typst.toml` 中的入口文件已更新
  - 示例内容中的导入路径：已更新为使用新的模板名称
  - 样式文件名称保持不变（仍为 `tufted.css`）

- 更新后的文件结构
  - 主模板：`src/twilightpage.typ`
  - 配置文件：`template/config.typ`
  - 包元数据：`typst.toml`
  - 示例内容：`template/content/index.typ` 等

- 使用注意事项
  - 现有项目中的导入语句需要更新为使用 `twilightpage` 模块
  - 示例内容中的 `#import "../config.typ": template, twilightpage` 语句已正确更新
  - 构建脚本和 Makefile 无需修改，因为它们引用的是配置文件而非模板文件
