...
## 更新摘要
**变更内容**
- 主入口函数从 `tufted-web` 重命名为 `twilightpage-web`
- 包文件从 `tufted.typ` 重命名为 `twilightpage.typ`
- 模板导入路径从 `@preview/tufted:0.1.1` 更新为 `@preview/twilightpage:0.1.1`
- 样式文件仍保持 `tufted.css` 名称但属于 TwilightPage 项目
- 所有相关示例和文档中的文件引用已更新
...
## 引言
本文件系统性解析 TwilightPage 模板系统的工作原理与架构设计，重点覆盖以下方面：
- 模板函数的定义与调用机制：包括主入口函数 twilightpage-web 的作用、参数配置与渲染流程。
...
graph TB
subgraph "包层src/"
A["twilightpage.typ<br/>主入口与全局样式/头部/主体拼装"]
...
**图表来源**
- [src/twilightpage.typ:17-63](file://src/twilightpage.typ#L17-L63)
...
## 核心组件
- 主入口函数 twilightpage-web：负责组装 HTML 文档骨架（head/body）、注入样式表、设置语言、渲染头部导航与正文内容，并串联各子模板。
- 子模板：
...
participant Tpl as "src/twilightpage.typ"
...
Typ->>Cfg : 解析模板入口
Cfg->>Tpl : 实例化 twilightpage-web.with(...)
...
```mermaid
flowchart TD
Start(["进入 twilightpage-web"]) --> SetLang["设置语言属性"]
...
## 依赖关系分析
- 包层内部依赖
  - twilightpage-web 依赖 layout、math、refs、notes、figures。
  - figures 依赖 layout（margin-note）。
- 模板层依赖
  - config.typ 依赖包层入口 twilightpage。
...
**章节来源**
- [src/twilightpage.typ:1-64](file://src/twilightpage.typ#L1-L64)
- [src/figures.typ:1-20](file://src/figures.typ#L1-L20)
- [template/config.typ:3-11](file://template/config.typ#L3-L11)
- [template/Makefile:1-27](file://template/Makefile#L1-L27)
...
## 结论
TwilightPage 模板系统通过"包层组件 + 模板层配置 + 构建脚本"的分层设计，实现了高内聚、低耦合的页面生成体系。主入口函数 twilightpage-web 作为装配中心，将数学、引用、脚注、图注与布局工具有机整合；模板层通过 with(...) 提供灵活的参数覆写；构建脚本自动化地将内容编译为 HTML 并输出静态资源。该架构既便于扩展（新增子模板或覆写渲染规则），也便于维护（集中式样式与统一的 HTML 结构）。
