...
subgraph "模板与内容"
L["template/Makefile<br/>编译 .typ → .html"]
...
- Makefile（模板）：将内容目录下的 .typ 编译为 HTML，复制静态资源至 _site。
...
- 模板 Makefile
  ...
  - 编译规则：typst compile --root .. --features html --format html。
...
## 结论
TwilightPage 的部署体系以 GitHub Actions 为核心，结合 Makefile 实现内容到静态站点的高效转换，并通过工件复用同时支持 GitHub Pages 与 Cloudflare Pages 的发布。通过规范化的权限、明确的触发条件与资源压缩策略，可在保障质量的同时提升发布效率。建议后续引入缓存与资源优化策略，持续改进性能与可靠性。
...
章节来源
- [Makefile:54-55](file://Makefile#L54-L55)
- [template/README.md:15-19](file://template/README.md#L15-L19)
...
章节来源
- [template/content/docs/04-deploy/index.typ:54-60](file://template/content/docs/04-deploy/index.typ#L54-L60)
